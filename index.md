...
Over the years a lot of good methods for low-light image enhancement have been developed. The methods to do so thus actually exist but are often focussed on targeting restoration quality, this comes at the expense of computational speed and that makes most of the existing methods for low-light image enhancement non-practical solutions [1]. In the paper "Restoring Extremely Dark Images in Real-Time" (M. Lamba & K. Mitra, 2021) a fast and memory efficient solution is presented which at the same time produces proper quality (and light enhanced) images. When running the author's code most noticeably to us was the fact that author's code mainly seemed to improve the image quality. The input images used by the researchers were often a bit noisy and dark whereas the output images seemed to be drastically denoised. The denoising/quality enhancing capabilities (in a high speed manner) of their model seem to be one of the model's key advantages.
...
1) Adding noise to the input image (in the pre-processing phase) and see it's influence on the output quality of the researchers model.
...
<p align = "center">
<b> Fig.N2 - Original unprocessed image opened with rawpy postprocess (left), original unprocessed image opened with our parameters (middle) and the image after adding a noise of variance 0.1 on the red, green and blue channels (right). All images shown are from before being restored by the network. </b>
</p>
...
<p align = "center">
<b> Fig.N4 - Original unprocessed image opened with rawpy postprocess (left), original unprocessed image opened with our parameters (middle) and the image after adding a noise of variance 0.5 on the red, green and blue channels (right). All images shown are from before being restored by the network. </b>
</p>
...
In our work we have tried and tested different images and noise settings with similar results. This means that for the addition of noise it seems that the claimed good generalization without new training could not be reproduced.
...
<p align = "center">
<b> Fig.N9 - Restored images after feeding the images with increased brigthness pixel value of 50 on the LHS half of the picture </b>
</p>


## Discussion
In this reproducibility project two different types of variances (noise and brightness) are added to the input images in order to check the robustness of the model produced by the researchers. The addition of noise showed a very disturbed and unrealistic output image which indicates that the researches model is not well resistant to noise variances. This may be due to the researchers model being trained on high quality images (also containing little noise), possibly also having very little difference in qualities between the images in that speficic training dataset. We suspect the model to be so well trained to a specific type of image quality that the disturbed results we got are due to the model not being able to handle the added random noise variance. 

The brightness addition also showed a decrease in the model's performance, dark parts of the images did become more visible but the overall brightness and image quality was substantialy lower compared to the output obtained when using the researcher's "original" input images. We believe the reason for this decrease in performance would mostly likely be due to a training dataset which has not been properly diversified in terms of containing images with different degrees in brightness. This could explain the decrease of the model's performance when presented with higher brightness input images.

## Conclusion
Returning to the motivation of our research namely testing the model's robustness and checking whether this research actually provides a smart solution or if it's rather a form of parameter tuning and overfitting that has taken place? Looking at our research goals, we found that addition of noise to the input image impacted the output images in a really negative way. This may imply that the model shows signs of overfitting and parameter tuning. At the same time this may also be the result of a not properly diversified training dataset being used by the reseachers. The addition of brightness to the input images returned output images with a decreased performance (less brightness and decreased visabilty) compared to the results obtained when using the input images provided by the reseachers. We believe this may suggest that the model is trained using dataset which has not been properly diversified in terms of containing images with different degrees in brightness.

In short, the results obtained from the images with added variances show a decrease in overall output quality. It showed the noise and brightness addition being badly processed by the model. For now we conclude the model not being very robust mostly because of it's bad performance when noise or brightness is added. Relevant additional research may be done on verifying to what extend the images from the training dataset are diversified in term of different images qualities (e.g. different degrees of noise) and checking wheter the magnitude of brightness also differs within the training dataset. In case the above is not the case, it would be interesting to see how to model's robustness performs when the model is retrained using a well diversified training dataset.
